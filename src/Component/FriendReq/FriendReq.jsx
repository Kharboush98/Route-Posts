...
export default function FriendReq() {
    ...
    useEffect(()=> {
        if(profileData?._id)
            {
                getFollowersSuggestion(profileData?._id);
            }
    },[profileData]
)
                ...
                <div key={_user?._id} className="space-y-3">
                    <div className="rounded-xl border border-slate-200 p-2.5">
                        ...
                        <div className='mt-2 flex items-center gap-2 text-[12px] font-semibold text-slate-500'>
                            ...
                            <span className='rounded-full bg-slate-100 px-2 py-0.5'>{_user?.followersCount} Followers</span>
                            :
                            <></>
                            }
                            
                            {_user?.mutualFollowersCount > 0 ? 
                            <span className='rounded-full bg-[#edf4ff] px-2 py-0.5 text-[#1877f2]'>{_user?.mutualFollowersCount} Mutual</span>
                            :
                            <></>
                            }                            
                        </div>
